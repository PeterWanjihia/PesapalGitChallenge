...
int main() {
    ...
    // Test file 1: test.txt
    strcpy(file_path, "test.txt");
    generate_hash(file_path, file_hash);

    // Log and store the file
    log_message("Adding file 'test.txt' to staging area.");
    store_in_staging_area(file_path, file_hash);

    // Simulate file modification (e.g., re-generate the hash)
    printf("\n--- Modifying the file ---\n");
    generate_hash(file_path, file_hash);

    // Log and store the updated file
    log_message("Modifying file 'test.txt' and updating hash in staging area.");
    store_in_staging_area(file_path, file_hash);
